...
static void buildPositionRemap(datatype_t* remap, const real_t* vertex_positions, size_t vertex_count, size_t vertex_positions_stride, meshopt_Allocator& allocator)
{
	VertexHasher vertex_hasher = {reinterpret_cast<const unsigned char*>(vertex_positions), 3 * sizeof(real_t), vertex_positions_stride};
	...
	for (size_t i = 0; i < vertex_count; ++i)
	{
		...
		datatype_t* entry = hashLookup(vertex_table, vertex_table_size, vertex_hasher, index, ALL_BITS_ONE);
...
size_t meshopt_generateVertexRemap(datatype_t* destination, const datatype_t* indices, size_t index_count, const void* vertices, size_t vertex_count, size_t vertex_size)
{
	...
	for (size_t i = 0; i < index_count; ++i)
	{
		...
		{
			datatype_t* entry = hashLookup(table, table_size, hasher, index, ALL_BITS_ONE);
			...
			{
				assert(destination[*entry] != ALL_BITS_ONE);
...
void meshopt_remapIndexBuffer(datatype_t* destination, const datatype_t* indices, size_t index_count, const datatype_t* remap)
{
	...
	{
		datatype_t index = indices ? indices[i] : datatype_t(i);
		assert(remap[index] != ALL_BITS_ONE);
...
void meshopt_generateAdjacencyIndexBuffer(datatype_t* destination, const datatype_t* indices, size_t index_count, const real_t* vertex_positions, size_t vertex_count, size_t vertex_positions_stride)
{
	using namespace meshopt;
...
void meshopt_generateTessellationIndexBuffer(datatype_t* destination, const datatype_t* indices, size_t index_count, const real_t* vertex_positions, size_t vertex_count, size_t vertex_positions_stride)
{
	using namespace meshopt;
